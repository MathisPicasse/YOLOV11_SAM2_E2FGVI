...
class Masker:
    ...
    def model(self, model_name: str):
        ...
        if not os.path.isfile(path_to_model):
            ...
            raise FileNotFoundError(
                f"Semantic model file does not exist: {path_to_model}")
    ...
    def create_mask(self, frame: np.ndarray, observations: List[Observation], output_path: str, mask_name) -> np.ndarray:
        ...
        if mask_obj is None or mask_obj.data is None:
            ...
            logger.warning(
                f"No mask data found by SAM for {frame_id_info} and bboxes {bboxes}")
            return np.zeros(frame.shape[:2], dtype=np.uint8)

        # Shape (N, H, W), values are float (0.0 to 1.0)
